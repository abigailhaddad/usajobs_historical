...
```{python}
# Analyze by position title
position_summary = scraped_enriched.groupby('position_title').agg({
    'questionnaire_id': 'count',
    'has_executive_order': 'sum'
}).reset_index()

position_summary.columns = ['Position_Title', 'Total_Scraped', 'Count_With_EO']
position_summary['Percent'] = (position_summary['Count_With_EO'] / position_summary['Total_Scraped'] * 100).round(0).astype(int)

# Sort by count with EO mentions
position_summary = position_summary.sort_values('Count_With_EO', ascending=False)

# Display position title table
print("Position titles with the 'advancing President's Executive Orders' question:")
position_summary[['Position_Title', 'Total_Scraped', 'Count_With_EO', 'Percent']].head(20)
```
...
detail_table = eo_jobs[['position_title', 'hiring_agency', 'job_posting_link', 'questionnaire_link']].copy()
detail_table.columns = ['Position_Title', 'Agency', 'Job_Posting_Link', 'Questionnaire_Link']

# Sort by position title
detail_table = detail_table.sort_values('Position_Title')
