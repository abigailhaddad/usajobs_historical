...
# Load service type data from current jobs parquet
print("Loading service type data from parquet files...")
current_jobs_2025 = pd.read_parquet(DATA_DIR / 'current_jobs_2025.parquet')

# Create a lookup for service type by control number
service_lookup = current_jobs_2025.set_index('usajobsControlNumber')['serviceType'].to_dict()

# Add service type to links_df
links_df['service_type'] = links_df['usajobs_control_number'].astype(str).map(service_lookup)
