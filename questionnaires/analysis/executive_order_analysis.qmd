...
if 'position_open_date' in links_df_preview.columns:
    links_df_preview['position_open_date'] = pd.to_datetime(links_df_preview['position_open_date'])
...
# Display occupation series table
print("Occupation series with the 'advancing President's Executive Orders' question:")
occupation_summary[['Occupation', 'Total_Scraped', 'Count_With_EO', 'Percent']].head(20)
```
...
scraped_df['posted_month'] = pd.to_datetime(scraped_df['position_open_date']).dt.to_period('M')
...
if missing_questionnaires:
    ...
    display(HTML(styled_failed.to_html(escape=False)))
else:
