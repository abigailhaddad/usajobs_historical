...
def check_no_job_id_loss():
    ...
    Reads only the control-number column to avoid OOM on large parquets.
    Warns loudly if a baseline-tracked file is missing from disk (not silently skipped).
    """
    ...
    for filename, baseline_ids in baseline.get('job_ids', {}).items():
        ...
        if not os.path.exists(filepath):
            if baseline_ids:
                print(f"{Colors.RED}❌ FAIL{Colors.RESET} {filename} is in baseline but missing from disk — file may have been lost from R2")
                all_good = False
            continue
        ...
        try:
            # Read only the ID column to avoid loading the full (potentially GB-sized) parquet
            current_ids = None
            for col in ('usajobsControlNumber', 'usajobs_control_number', 'PositionID'):
                try:
                    s = pd.read_parquet(filepath, columns=[col])[col]
                    current_ids = set(s.dropna().astype(str).unique())
                    break
                except Exception:
                    continue

            if current_ids is None:
                print(f"{Colors.YELLOW}⚠️  WARN{Colors.RESET} {filename} - no known ID column found")
                continue
...
def check_data_consistency():
    ...
    try:
        ...
        # Fall back to previous year if current year files don't exist
        if not os.path.exists(current_file):
            current_file = f'../data/current_jobs_{current_year - 1}.parquet'
...
def run_tests():
    ...
    for year in range(2026, current_year + 1):
        current_files.append((f'current_jobs_{year}.parquet', 1, ['positionTitle'], f'Current jobs {year}'))
