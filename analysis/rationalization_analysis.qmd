...
try:
    if 'scraped_sections' in historical_df.columns:
        scraped_jobs = len(historical_df[
            (historical_df['scraped_sections'].notna()) & 
            (historical_df['scraped_sections'] != '{}') &
            (historical_df['scraped_sections'] != '')
        ])
        print(f"🕷️ Scraped Jobs: {scraped_jobs:,} ({scraped_jobs/total_historical*100:.1f}%)")
    else:
        print("🕷️ Scraped Jobs: N/A (no scraped_sections column yet)")
        scraped_jobs = 0
except Exception as e:
    print(f"⚠️ Could not calculate scraped jobs coverage: {e}")
    scraped_jobs = 0
...
if not unified_df.empty:
    ...
    if field_coverage_data:
        ...
        print("📊 Field Coverage by Data Source Category (% of records with data):")
        print(pivot_df.to_string())
        
        # Show specific insights for key content fields
        print("\n🔍 Key Content Fields Analysis:")
