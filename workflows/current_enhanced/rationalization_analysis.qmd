...
conn = duckdb.connect("data/unified_minimal_test_20250612_131510.duckdb")
```
...
print("🔍 SIDE-BY-SIDE COMPARISON: SAME JOBS IN BOTH DATASETS")
print("=" * 80)
...
if overlap_jobs:
    ...
    for i, (control_num,) in enumerate(overlap_jobs, 1):
        ...
        for j, field_name in enumerate(field_names):
            hist_val = hist_job[j] if hist_job else "❌ Missing"
            curr_val = curr_job[j] if curr_job else "❌ Missing"
            ...
            def format_val(val):
                if val is None:
                    ...
                    return f"${val:,.0f}" if val else "❌ None"
                else:
                    val_str = str(val)
    ...
    print("❌ No overlapping jobs found between historical and current datasets")
```
...
if field_coverage:
    ...
    for field_name, idx in field_names:
        ...
        if idx == 1:  # Total records
            print(f"{field_name:<20} {hist_val:<15,} {curr_val:<15,} {'📊 Counts'}")
        else:
            ...
            if hist_pct > curr_pct + 10:
                comparison = "🟢 Hist Better"
            elif curr_pct > hist_pct + 10:
                comparison = "🔵 Curr Better"  
...
print(f"\n🔍 KEY INSIGHTS:")
print(f"• Historical API: Structured data with consistent core fields")
print(f"• Current API: Rich content fields like duties and qualifications") 
print(f"• Rationalization: Combines best of both sources")
```
...
print("🏷️ Most Common Position Titles:")
...
if consistency_check:
    print(f"\n⚠️ Agencies with Multiple Departments (potential inconsistencies):")
...
print(f"📊 Job Series Completeness:")
...
print("📝 Major Duties Field Analysis:")
...
print("🎯 Rationalization Confidence Distribution:")
...
print("⭐ Sample High-Quality Rationalized Records:")
print()
...
print("🗺️ Geographic Distribution of Jobs:")
...
print("🎯 RATIONALIZATION QUALITY SUMMARY")
print("=" * 50)
print(f"✅ High Confidence Records (≥0.90): {total_high_quality:,} ({(total_high_quality/total_records)*100:.1f}%)")
print(f"✅ Complete Core Fields: {total_complete_core:,} ({(total_complete_core/total_records)*100:.1f}%)")
print(f"✅ Average Confidence Score: {conn.execute('SELECT AVG(confidence_score) FROM unified_jobs').fetchone()[0]:.3f}")

# Close connection
conn.close()

print(f"\n🔍 Key Findings:")
print(f"• Position titles are well-preserved with high completeness")
print(f"• Agency names show good consistency across departments") 
print(f"• Job series codes are properly rationalized for most records")
print(f"• Major duties field needs enhancement from scraping for completeness")
print(f"• Overall data quality is good with strong confidence scores")
```