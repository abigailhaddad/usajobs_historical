...
total_historical = len(historical_df)
total_current = len(current_df)
total_unified = len(unified_df)

print(f"📊 Historical Jobs: {total_historical:,}")
print(f"📊 Current Jobs: {total_current:,}")
print(f"📊 Unified Jobs: {total_unified:,}")

# Check scraping coverage
scraped_jobs = len(historical_df[historical_df['scraped_sections'] != '{}'])
print(f"🕷️ Scraped Jobs: {scraped_jobs:,} ({scraped_jobs/total_historical*100:.1f}%)")
...
try:
    ...
    print(f"🔄 Overlap Jobs: {overlap_jobs:,}")
except:
    print("🔄 Overlap Jobs: 0 (no overlap samples found)")
```
            ...
            if 'historical' in sources and 'current' in sources and comparison_count < 3:
                ...
                for field_name in key_fields:
                    if field_name in hist_data.index:
                        hist_val = hist_data[field_name] if pd.notna(hist_data[field_name]) and hist_data[field_name] != '' else "❌"
                        curr_val = curr_data[field_name] if pd.notna(curr_data[field_name]) and curr_data[field_name] != '' else "❌"
                        ...
                        # Mark differences
                        match_indicator = "✅" if str(hist_val) == str(curr_val) else "❓"
...
if not unified_df.empty:
    ...
    if hist_total > 0:
        for field_name in analysis_fields:
            if field_name in ['min_salary', 'max_salary', 'total_openings']:
                ...
                complete_count = hist_df[field_name].notna().sum()
                ...
                complete_count = ((hist_df[field_name].notna()) & (hist_df[field_name] != '')).sum()
            
            percentage = round(complete_count * 100.0 / hist_total, 1) if hist_total > 0 else 0
    ...
    if curr_total > 0:
        for field_name in analysis_fields:
            if field_name in ['min_salary', 'max_salary', 'total_openings']:
                # Numeric fields - check for non-null values
                complete_count = curr_df[field_name].notna().sum()
            elif field_name in ['open_date', 'close_date']:
                # Date fields - check for non-null values
                complete_count = curr_df[field_name].notna().sum()
            else:
                # String fields - check for non-null and non-empty values
                complete_count = ((curr_df[field_name].notna()) & (curr_df[field_name] != '')).sum()
            
            percentage = round(complete_count * 100.0 / curr_total, 1) if curr_total > 0 else 0
    ...
    if field_coverage_data:
        ...
        print("📊 Field Coverage Comparison (% of records with data):")
        print(pivot_df.to_string())
        
        # Highlight fields with significant differences
        print("\n🔍 Fields with Major Coverage Differences:")
...
if not unified_df.empty:
    ...
    print(f"📊 Total Fields: {len(unified_df.columns)}")
